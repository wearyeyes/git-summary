...
$ git log --oneline
```
**??** - файл новый и не добавлен в отслеживаемые (`staging area`).
**A** - добавлен в отслеживаемые. 
**M** - в файл были внесены изменения.
...
Если вы забыли добавить что-то в последний коммит, то можете воспользоваться ключом `--amend` (будут добавлены изменения из индекса):
```
$ git commit --amend
```

Для изменения комментария в последнем коммите:
```
$ git commit --amend -m “new description”
```

Для исключения файла из индекса воспользуйтесь:
```
$ git reset HEAD <file>
```

Можно отменить сделанные изменения в файле (восстановления до состояния как в последнем коммите):
```
$ git checkout -- <file>
```
...
```
На самом деле таким образом создается новый указатель на текущий коммит. И вы все ещё находитесь на текущей ветке.
...
```
В результате указатель HEAD переместится на указанную ветку. 

Чтобы создать новую ветку и сразу же на нее переключиться:
```
$ git checkout -b <new-branch-name>
```

А эта команды выведит список веток проекта:
```
$ git branch
```

Чтобы увидеть последние коммиты на каждой из веток добавьте ключ -v:
```
$ git branch -v
```
...
git log по умолчанию отображает историю коммитов только для текущей ветки. Чтобы узнать информацию по всем веткам введите:
```
$ git log --all
```
либо конкретной ветки:
```
$ git log <branch-name>
```

Для отображения истории коммитов, текущего положения указателей веток и истории ветвления в виде графа выполните команду:
```
...
Если попытаться удалить ветку с ещё не слитыми наработками, git выведет сообщение об ошибке. В таком случае нужно воспользоваться командой:
```
$ git branch -D <branch-name>
```
...
В ходе работы может возникнуть ситуация, когда необходимо срочно переключиться с одной ветки на другую, но при этом сделанных изменений не достаточно для коммита. В таком случае можно загрузить эти файлы во временное хранилище - Stash - и при необходимости вернуться к ним позже. 
...
$ git stash list
```
Применить только что спрятанные изменения:
```
$ git stash apply
```
Применить конкретное спрятанное изменение:
```
$ git stash apply stash@{n}
```
Apply применяет указанные изменения, но не удаляет их из хранилища. Чтобы удалить:
```
$ git stash drop stash@{n}
```
Применить изменения и тут же удалить:
```
$ git stash pop
```

Чтобы не прятать в хранилище файлы уже добавленные в индекс:
```
$ git stash --keep-index
```
По умолчанию прячутся только файлы уже добавленные в индекс. Добавить и неотслеживаемые файлы можно с помощью `--include-untracked` или `-u`:
```
$ git stash -u
```
С флагом `--patch` git спросит вас что добавлять, а что - нет:
```
$ git stash --patch
```
